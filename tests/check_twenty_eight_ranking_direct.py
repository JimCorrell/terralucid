...
from refresh_twenty_eight_correction_ranking import BASE,ROOT
...
checks=[]
targets=[r['ranking'][0]['object_id']]+[x['object_id'] for x in sorted(r['ranking'],key=lambda x:-x['additional_reduction_since_seventeen_corrections_m2'])[:2]]
...
assert len(r['ranking'])==134 and len({x['object_id'] for x in r['ranking']})==134
assert all(x['envelope_uncovered_county_m2']<=x['previous_envelope_uncovered_county_m2'] for x in r['ranking'])
assert len(cs)==28
assert all(x['envelope_uncovered_county_m2']<=x['seventeen_correction_envelope_uncovered_county_m2']+0.01 for x in r['ranking'])
assert not set(r['removed_accepted_object_ids']) & {x['object_id'] for x in r['ranking']}
assert abs(r['additional_coverage_since_seventeen_corrections_m2']-87000481.79749993)<0.01
(BASE/'test-results.json').write_text(json.dumps({'passed':True,'unit_tests':5,'checks':['overlapping candidates counted once; holes, disjoint parts and boundary touches','stale/changed dependencies and changed hold cohort rejected','multiple civil records per jurisdiction aggregated','134 unique remaining holds; monotonic upper bounds','new coverage agrees with eleven-proposal scenario within 0.01 square metre'],'direct_overlay_checks':checks},indent=2,sort_keys=True)+'\n')
print(checks)
