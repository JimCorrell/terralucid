...
proposal=json.loads(Path('research/three-zoning-priorities/report.json').read_text())
activation=json.loads(Path('research/three-zoning-acceptance/report.json').read_text())
...
run(Path('supabase/migrations/20260921070000_three_zoning_acceptance.sql').read_text())
...
check(run('SELECT count(*) FROM ingest.county_geometry_correction')=='5','failed baseline load leaked writes')
...
run(load);run(load);check(run('SELECT count(*) FROM ingest.county_geometry_event')=='8','exact replay duplicated events');checks.append('authentic eight-feature activation and full load replay');check(run(old_state_sql)==old_state,'prior five corrections changed');check(run("SELECT needs_revisit FROM ingest.county_geometry_snapshot_current WHERE snapshot_id='"+old_snapshot+"';")=='t','old snapshot did not become stale');check(run("SELECT count(*) FROM ingest.county_geometry_snapshot_current WHERE NOT needs_revisit")=='1','new snapshot missing');checks.append('prior five approvals unchanged; old snapshot stale and new snapshot current')
check(run("SELECT ingest.county_geometry_snapshot_is_current(document->>'geometry_snapshot_id',document->>'source_audit_sha256') FROM ingest.audit_result WHERE sha256='07efac22afa8829de039ada598fa06533f378c33edc720f40a255b67c75d0136'")== 'f','old ranking is not stale')

for oid in [27304757,27270812,27207268]:run(Path('tests/three_zoning_acceptance.sql').read_text().replace('TEST_OBJECT_ID',str(oid)))
...
report=json.loads(Path('research/three-zoning-acceptance/report.json').read_text());c=report['corrections'][0];cid=c['correction_id'];eid=c['acceptance']['event_id'];audit=report['source_audit_sha256']
