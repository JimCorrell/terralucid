...
from refresh_seventeen_correction_ranking import BASE,ROOT,ACTIVATIONS,validate_dependencies

class SeventeenRankingTests(prior.RankingTests):
    def test_live_cohort_dependencies(self):
        ...
        self.assertEqual(len(cs),17);self.assertEqual(len(d['held_object_ids']),145)
