-- Run only in a disposable database containing the authentic county + twenty-eight acceptances.
BEGIN;
...
END $$;
SELECT pg_temp.assert_true((SELECT count(*) FROM ingest.county_inventory_feature)=120449
 AND (SELECT count(*) FROM ingest.county_inventory_feature WHERE geometry IS NULL)=183
 AND count(*) FILTER(WHERE effective_geometry_hold IS NOT NULL)=155
 AND count(*) FILTER(WHERE source_id='lupc-zoning' AND effective_geometry_hold IS NOT NULL)=134
 AND NOT bool_or(qualified_for_parcel_screening),'inventory counts/qualification')
 FROM ingest.effective_county_inventory WHERE original_geometry IS NULL;
SELECT pg_temp.assert_true(count(*)=28 AND bool_and(extensions.ST_IsValid(effective_geometry))
...
CREATE TEMP TABLE fixture AS SELECT c.*,ingest.record_county_geometry_snapshot(c.source_audit_sha256) sid
 FROM ingest.county_geometry_current c WHERE object_id=TEST_OBJECT_ID;
...
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state)
 ...
 FROM ingest.county_geometry_correction WHERE object_id=TEST_OBJECT_ID LIMIT 1 ON CONFLICT DO NOTHING$q$,'replay differs');
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state)
...
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state)
 ...
 FROM ingest.county_geometry_correction WHERE object_id=TEST_OBJECT_ID LIMIT 1 ON CONFLICT DO NOTHING$q$,'Outside reviewed county cohort');
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state)
 SELECT repeat('0',64),source_audit_sha256,source_id,99999999,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state
 FROM ingest.county_geometry_correction WHERE object_id=TEST_OBJECT_ID LIMIT 1 ON CONFLICT DO NOTHING$q$,'Outside reviewed county cohort');
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state)
 ...
 FROM ingest.county_geometry_correction WHERE object_id=TEST_OBJECT_ID LIMIT 1 ON CONFLICT DO NOTHING$q$,'differs from exact reviewed source/candidate');
...
SELECT pg_temp.assert_true(correction_status='withdrawn','historical replay does not reactivate') FROM ingest.county_geometry_current WHERE object_id=TEST_OBJECT_ID;
...
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state) SELECT repeat('0',64),source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,'25f03b37818d1fc5cf25a6fa1e9264d48824c8585dbbf9776574dd6682905b6c','b7d02438499be17b55445bb5b2c5cd621cbe6f24182d238569524e0c9ab20cb7',candidate_text,candidate_sha256,evidence_state FROM ingest.county_geometry_correction WHERE object_id=27208875$q$,'Outside reviewed county cohort');
SELECT pg_temp.rejects($q$INSERT INTO ingest.county_geometry_correction(correction_id,source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,proposal_audit_sha256,review_audit_sha256,candidate_text,candidate_sha256,evidence_state) SELECT repeat('0',64),source_audit_sha256,source_id,object_id,source_input_sha256,source_feature_text,'959a971c95dc6e81a1179d26f345eba9dc2fe9b908c40107f43272b27ef5d49a','6a649c0cfffedf0fde0d3790a6f44fc8a0990baa9e5e5b7e705f7569e513900a',candidate_text,candidate_sha256,evidence_state FROM ingest.county_geometry_correction WHERE object_id=27334504$q$,'Outside reviewed county cohort');
ROLLBACK;
