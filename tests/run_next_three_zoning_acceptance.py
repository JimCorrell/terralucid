...
proposal=json.loads(Path('research/next-three-zoning-priorities/report.json').read_text())
activation=json.loads(Path('research/next-three-zoning-acceptance/report.json').read_text())
...
check(len(statements)==3,'wrong insert cohort')
...
checks.append('previous validator rejects all three new exact versions')
run(Path('supabase/migrations/20260922010000_next_three_zoning_acceptance.sql').read_text())
...
check(run('SELECT count(*) FROM ingest.county_geometry_correction')=='8','failed baseline load leaked writes')
...
checks.append('fresh inserts enforce source/audit/candidate binding, cycle/fill evidence, runtime validity and exact segment multiplicity for all three cases')
...
run(load);run(load);check(run('SELECT count(*) FROM ingest.county_geometry_event')=='11','exact replay duplicated events');checks.append('authentic eleven-feature activation and full load replay');check(run(old_state_sql)==old_state,'prior eight corrections changed');check(run("SELECT needs_revisit FROM ingest.county_geometry_snapshot_current WHERE snapshot_id='"+old_snapshot+"';")=='t','old snapshot did not become stale');check(run("SELECT count(*) FROM ingest.county_geometry_snapshot_current WHERE NOT needs_revisit")=='1','new snapshot missing');checks.append('prior eight approvals unchanged; old snapshot stale and new snapshot current')
check(run("SELECT ingest.county_geometry_snapshot_is_current(document->>'geometry_snapshot_id',document->>'source_audit_sha256') FROM ingest.audit_result WHERE sha256='6b8896283cc6637d5d2dc77c51fe1fe28f2600f64c76ef25649f05893ad418eb'")== 'f','old ranking is not stale')

for oid in [27224591,27320676,27243652]:run(Path('tests/next_three_zoning_acceptance.sql').read_text().replace('TEST_OBJECT_ID',str(oid)))
...
report=json.loads(Path('research/next-three-zoning-acceptance/report.json').read_text());c=report['corrections'][0];cid=c['correction_id'];eid=c['acceptance']['event_id'];audit=report['source_audit_sha256']
