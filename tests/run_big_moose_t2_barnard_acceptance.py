...
proposal=json.loads(Path('research/big-moose-t2-barnard/report.json').read_text())
activation=json.loads(Path('research/big-moose-t2-barnard-acceptance/report.json').read_text())
...
run(Path('supabase/migrations/20260922030000_big_moose_t2_barnard_acceptance.sql').read_text())
...
check(run('SELECT count(*) FROM ingest.county_geometry_correction')=='14','failed baseline load leaked writes')
...
run(load);run(load);check(run('SELECT count(*) FROM ingest.county_geometry_event')=='17','exact replay duplicated events');checks.append('authentic seventeen-feature activation and full load replay');check(run(old_state_sql)==old_state,'prior fourteen corrections changed');check(run("SELECT needs_revisit FROM ingest.county_geometry_snapshot_current WHERE snapshot_id='"+old_snapshot+"';")=='t','old snapshot did not become stale');check(run("SELECT count(*) FROM ingest.county_geometry_snapshot_current WHERE NOT needs_revisit")=='1','new snapshot missing');checks.append('prior fourteen approvals unchanged; old snapshot stale and new snapshot current')
check(run("SELECT ingest.county_geometry_snapshot_is_current(document->>'geometry_snapshot_id',document->>'source_audit_sha256') FROM ingest.audit_result WHERE sha256='3dd7c8ccf884b20e49fae2f0949b047cf7635cea978ea287053b5a6ab96ef10d'")== 'f','old ranking is not stale')

for oid in [27257109,27321193,27303187]:run(Path('tests/big_moose_t2_barnard_acceptance.sql').read_text().replace('TEST_OBJECT_ID',str(oid)))
...
report=json.loads(Path('research/big-moose-t2-barnard-acceptance/report.json').read_text());c=report['corrections'][0];cid=c['correction_id'];eid=c['acceptance']['event_id'];audit=report['source_audit_sha256']
