-- Run only in a disposable database containing the authentic county + eight acceptances.
BEGIN;
...
SELECT pg_temp.assert_true((SELECT count(*) FROM ingest.county_inventory_feature)=120449
 ...
 AND count(*) FILTER(WHERE effective_geometry_hold IS NOT NULL)=175
 AND count(*) FILTER(WHERE source_id='lupc-zoning' AND effective_geometry_hold IS NOT NULL)=154
...
SELECT pg_temp.assert_true(count(*)=8 AND bool_and(extensions.ST_IsValid(effective_geometry))
