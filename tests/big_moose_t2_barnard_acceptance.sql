-- Run only in a disposable database containing the authentic county + seventeen acceptances.
BEGIN;
...
SELECT pg_temp.assert_true((SELECT count(*) FROM ingest.county_inventory_feature)=120449
 ...
 AND count(*) FILTER(WHERE effective_geometry_hold IS NOT NULL)=166
 AND count(*) FILTER(WHERE source_id='lupc-zoning' AND effective_geometry_hold IS NOT NULL)=145
...
SELECT pg_temp.assert_true(count(*)=17 AND bool_and(extensions.ST_IsValid(effective_geometry))
