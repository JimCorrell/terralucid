-- Run only in a disposable database containing the authentic county + eleven acceptances.
BEGIN;
...
SELECT pg_temp.assert_true((SELECT count(*) FROM ingest.county_inventory_feature)=120449
 ...
 AND count(*) FILTER(WHERE effective_geometry_hold IS NOT NULL)=172
 AND count(*) FILTER(WHERE source_id='lupc-zoning' AND effective_geometry_hold IS NOT NULL)=151
...
SELECT pg_temp.assert_true(count(*)=11 AND bool_and(extensions.ST_IsValid(effective_geometry))
