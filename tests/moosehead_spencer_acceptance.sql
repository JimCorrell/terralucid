-- Run only in a disposable database containing the authentic county + five acceptances.
BEGIN;
...
SELECT pg_temp.assert_true(count(*)=120449 AND count(*) FILTER(WHERE original_geometry_hold IS NOT NULL)=183
 AND count(*) FILTER(WHERE effective_geometry_hold IS NOT NULL)=178
 AND count(*) FILTER(WHERE source_id='lupc-zoning' AND effective_geometry_hold IS NOT NULL)=157
 AND NOT bool_or(qualified_for_parcel_screening),'inventory counts/qualification') FROM ingest.effective_county_inventory;
SELECT pg_temp.assert_true(count(*)=5 AND bool_and(extensions.ST_IsValid(effective_geometry))
