...
from refresh_eleven_correction_ranking import BASE,ROOT,ACTIVATIONS,validate_dependencies

class ElevenRankingTests(prior.RankingTests):
    def test_live_cohort_dependencies(self):
        ...
        self.assertEqual(len(cs),11);self.assertEqual(len(d['held_object_ids']),151)
