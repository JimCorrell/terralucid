...
proposal=json.loads(Path('research/latest-three-zoning-priorities/report.json').read_text())
activation=json.loads(Path('research/latest-three-zoning-acceptance/report.json').read_text())
...
run(Path('supabase/migrations/20260922020000_latest_three_zoning_acceptance.sql').read_text())
...
check(run('SELECT count(*) FROM ingest.county_geometry_correction')=='11','failed baseline load leaked writes')
...
run(load);run(load);check(run('SELECT count(*) FROM ingest.county_geometry_event')=='14','exact replay duplicated events');checks.append('authentic fourteen-feature activation and full load replay');check(run(old_state_sql)==old_state,'prior eleven corrections changed');check(run("SELECT needs_revisit FROM ingest.county_geometry_snapshot_current WHERE snapshot_id='"+old_snapshot+"';")=='t','old snapshot did not become stale');check(run("SELECT count(*) FROM ingest.county_geometry_snapshot_current WHERE NOT needs_revisit")=='1','new snapshot missing');checks.append('prior eleven approvals unchanged; old snapshot stale and new snapshot current')
check(run("SELECT ingest.county_geometry_snapshot_is_current(document->>'geometry_snapshot_id',document->>'source_audit_sha256') FROM ingest.audit_result WHERE sha256='5510a6d2d39e371d8456d68201970f047b6e8893c8fca0dfffc8b21ddf1a6e27'")== 'f','old ranking is not stale')

for oid in [27233351,27314083,27316137]:run(Path('tests/latest_three_zoning_acceptance.sql').read_text().replace('TEST_OBJECT_ID',str(oid)))
...
report=json.loads(Path('research/latest-three-zoning-acceptance/report.json').read_text());c=report['corrections'][0];cid=c['correction_id'];eid=c['acceptance']['event_id'];audit=report['source_audit_sha256']
