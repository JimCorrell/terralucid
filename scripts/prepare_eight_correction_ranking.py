...
from refresh_eight_correction_ranking import BASE,AUDIT,OLD_SHA
...
def stage(output):
    ...
    for p in [ROOT/r['method'],Path(__file__),BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_eight_correction_ranking.py',ROOT/'tests/test_county_ranking_refresh.py',ROOT/'tests/check_eight_ranking_direct.py',ROOT/'scripts/prepare_county_ranking_refresh.py',registry/'registry.json']:archive(p.read_bytes())
