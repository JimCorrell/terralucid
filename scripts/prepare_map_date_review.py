#!/usr/bin/env python3
"""Stage seven map-date reviews and official evidence; never replace source values."""
...
from investigate_map_dates import BASE,AUDIT,RANKING_SHA,MAPS
...
def stage(output):
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','scope':'Seven map-date reviews; no source-value, geometry or qualification changes','geometry_snapshot_id':r['geometry_snapshot_id']})])+') ON CONFLICT DO NOTHING;')
