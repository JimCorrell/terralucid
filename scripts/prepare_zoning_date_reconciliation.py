#!/usr/bin/env python3
"""Stage two signed-decision reconciliations; preserve source values and open filing variances."""
...
from reconcile_zoning_dates import BASE,AUDIT,RANKING_SHA,MAPS,PRIOR_SHA,validate_review
...
def stage(output):
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_zoning_date_reconciliation.py']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','scope':'Two signed-decision dates; filing variance remains open; no source-value or geometry changes','geometry_snapshot_id':r['geometry_snapshot_id']})])+') ON CONFLICT DO NOTHING;')
