#!/usr/bin/env python3
"""Versioned ranking refresh after fourteen accepted county corrections."""
import json
...
BASE=ROOT/'research/fourteen-correction-ranking'
...
PREVIOUS_SHA='5510a6d2d39e371d8456d68201970f047b6e8893c8fca0dfffc8b21ddf1a6e27'
...
def run():
    ...
    previous=json.loads(read('research/eleven-correction-ranking/report.json',PREVIOUS_SHA))
    ...
    d=json.loads(read('research/fourteen-correction-ranking/dependencies.json'))
    ...
    for path,sha in ACTIVATIONS.items():
        ...
        for c in a['corrections']:
            ...
            parent={'county-corrections':'piscataquis-zoning-holds','moosehead-spencer-acceptance':'moosehead-spencer','three-zoning-acceptance':'three-zoning-priorities','next-three-zoning-acceptance':'next-three-zoning-priorities','latest-three-zoning-acceptance':'latest-three-zoning-priorities'}[Path(path).parent.name]
    ...
    for rank,r in enumerate(old['ranking'],1):
        ...
        if additional < -0.01:raise ValueError('Coverage regressed from eleven-correction ranking')
        rows.append({**r,'eleven_correction_rank':prior['rank'],'eleven_correction_envelope_uncovered_county_m2':prior['envelope_uncovered_county_m2'],'additional_reduction_since_eleven_corrections_m2':max(0.,additional),'previous_rank':rank,'previous_envelope_uncovered_county_m2':r['envelope_uncovered_county_m2'],'accepted_coverage_reduction_m2':reduction,'envelope_uncovered_county_m2':refreshed_area(r['envelope_uncovered_county_m2'],reduction),'potential_gap_by_civil_name_m2':jurisdictions})
    rows.sort(key=lambda r:(-r['envelope_uncovered_county_m2'],r['object_id']))
    for i,r in enumerate(rows,1):r['rank']=i
    report={'evidence_state':'DERIVED','method':'scripts/refresh_fourteen_correction_ranking.py','method_sha256':digest(Path(__file__).read_bytes()),'inputs':inputs,'source_audit_sha256':AUDIT,'historical_ranking_sha256':OLD_SHA,'eleven_correction_ranking_sha256':PREVIOUS_SHA,'additional_coverage_since_eleven_corrections_m2':sum(g.area for g in added)-previous['newly_covered_m2'],'geometry_snapshot_id':d['snapshot']['snapshot_id'],'geometry_dependencies':d['snapshot']['dependencies'],'runtime':{'shapely':shapely.__version__,'GEOS':shapely.geos_version_string},'original_held_zoning_rows':162,'effective_held_zoning_rows':len(rows),'removed_accepted_object_ids':sorted(c['object_id'] for c in corrections),'original_gap_m2':gap.area,'newly_covered_m2':sum(g.area for g in added),'effective_gap_m2':refreshed_area(gap.area,sum(g.area for g in added)),'qualified_for_parcel_screening':False,'ranking':rows,'limits':['T1 R11 map-index amendment date 2005-08-18 versus PDF ZP770 effective 2018-04-26 remains unresolved; geometry acceptance does not establish legal currency.','Envelope/gap overlap is an upper bound, not polygon coverage; priorities overlap and must not be added.','Inventory coverage includes retained source flags; a geometry gap does not establish absent zoning or municipal applicability.','This ranks geometry investigations, not parcels, suitability, legal zoning or acquisition readiness.','Original evidence is preserved; only the fourteen accepted exact versions affect this refresh. Check the snapshot against the intended audit before use.']}
