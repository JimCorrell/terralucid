#!/usr/bin/env python3
"""Stage the Orneville text-correction investigation without changing source values."""
...
from investigate_orneville_zp796 import BASE,AUDIT,RANKING_SHA,MAPS,PRIOR_SHA,validate_review

def stage(output):
    raw=(BASE/'report.json').read_bytes();r=json.loads(raw);audit=digest(raw)
    if r['source_audit_sha256']!=AUDIT or r['ranking_audit_sha256']!=RANKING_SHA or {c['map'] for c in r['cases']}!=MAPS:raise ValueError('Changed scope')
    validate_review(r)
    if r['prior_date_audit_sha256']!=PRIOR_SHA:raise ValueError('Changed prior date audit')
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_orneville_zp796.py']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','scope':'Orneville historical text correction established; current parcel applicability remains unknown; no geometry changes','geometry_snapshot_id':r['geometry_snapshot_id']})])+') ON CONFLICT DO NOTHING;')
