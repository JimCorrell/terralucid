#!/usr/bin/env python3
"""Archive the Moosehead/Spencer investigation; append findings, never accept geometry."""
import argparse,json
from pathlib import Path
from prepare_source_staging import ROOT,digest,canonical,literal,jsql,BUCKET
from investigate_moosehead_spencer import BASE,PARENT,PARENT_SHA


def stage(output):
    raw=(BASE/'report.json').read_bytes();r=json.loads(raw);audit=digest(raw)
    plan=json.loads((BASE/'finding-review.json').read_bytes())
    if r['prior_investigation_audit_sha256']!=PARENT_SHA or digest((PARENT/'report.json').read_bytes())!=PARENT_SHA:raise ValueError('Changed proposal audit')
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_moosehead_spencer.py',PARENT/'registry.json']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','proposal_audit_sha256':PARENT_SHA,'scope':'Two point-touching-hole proposals; original holds and existing acceptances unchanged'})])+') ON CONFLICT DO NOTHING;')
