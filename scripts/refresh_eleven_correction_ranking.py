#!/usr/bin/env python3
"""Versioned ranking refresh after eleven accepted county corrections."""
import json
...
BASE=ROOT/'research/eleven-correction-ranking'
...
PREVIOUS_SHA='6b8896283cc6637d5d2dc77c51fe1fe28f2600f64c76ef25649f05893ad418eb'
...
def run():
    ...
    previous=json.loads(read('research/eight-correction-ranking/report.json',PREVIOUS_SHA))
    ...
    d=json.loads(read('research/eleven-correction-ranking/dependencies.json'))
    ...
    for path,sha in ACTIVATIONS.items():
        ...
        for c in a['corrections']:
            ...
            parent={'county-corrections':'piscataquis-zoning-holds','moosehead-spencer-acceptance':'moosehead-spencer','three-zoning-acceptance':'three-zoning-priorities','next-three-zoning-acceptance':'next-three-zoning-priorities'}[Path(path).parent.name]
    ...
    for rank,r in enumerate(old['ranking'],1):
        ...
        if additional < -0.01:raise ValueError('Coverage regressed from eight-correction ranking')
        rows.append({**r,'eight_correction_rank':prior['rank'],'eight_correction_envelope_uncovered_county_m2':prior['envelope_uncovered_county_m2'],'additional_reduction_since_eight_corrections_m2':max(0.,additional),'previous_rank':rank,'previous_envelope_uncovered_county_m2':r['envelope_uncovered_county_m2'],'accepted_coverage_reduction_m2':reduction,'envelope_uncovered_county_m2':refreshed_area(r['envelope_uncovered_county_m2'],reduction),'potential_gap_by_civil_name_m2':jurisdictions})
    rows.sort(key=lambda r:(-r['envelope_uncovered_county_m2'],r['object_id']))
    for i,r in enumerate(rows,1):r['rank']=i
    report={'evidence_state':'DERIVED','method':'scripts/refresh_eleven_correction_ranking.py','method_sha256':digest(Path(__file__).read_bytes()),'inputs':inputs,'source_audit_sha256':AUDIT,'historical_ranking_sha256':OLD_SHA,'eight_correction_ranking_sha256':PREVIOUS_SHA,'additional_coverage_since_eight_corrections_m2':sum(g.area for g in added)-previous['newly_covered_m2'],'geometry_snapshot_id':d['snapshot']['snapshot_id'],'geometry_dependencies':d['snapshot']['dependencies'],'runtime':{'shapely':shapely.__version__,'GEOS':shapely.geos_version_string},'original_held_zoning_rows':162,'effective_held_zoning_rows':len(rows),'removed_accepted_object_ids':sorted(c['object_id'] for c in corrections),'original_gap_m2':gap.area,'newly_covered_m2':sum(g.area for g in added),'effective_gap_m2':refreshed_area(gap.area,sum(g.area for g in added)),'qualified_for_parcel_screening':False,'ranking':rows,'limits':['Envelope/gap overlap is an upper bound, not polygon coverage; priorities overlap and must not be added.','Inventory coverage includes retained source flags; a geometry gap does not establish absent zoning or municipal applicability.','This ranks geometry investigations, not parcels, suitability, legal zoning or acquisition readiness.','Original evidence is preserved; only the eleven accepted exact versions affect this refresh. Check the snapshot against the intended audit before use.']}
