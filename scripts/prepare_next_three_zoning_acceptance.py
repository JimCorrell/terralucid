...
BASE=ROOT/'research/next-three-zoning-acceptance'
COUNTY='fad71a48eab71fce398a8e51bd3fb4e9b7169e1d0e0f2a6ed91b1a18a5fad259'
PROPOSAL='a45d02d957bc6a5db61a53e8bf42be4000e582e08a07ea520f6e22254947b3be'
REVIEW='a45d02d957bc6a5db61a53e8bf42be4000e582e08a07ea520f6e22254947b3be'
MIGRATION='supabase/migrations/20260922010000_next_three_zoning_acceptance.sql'

def prepare(output,transfer):
    proposal_path=ROOT/'research/next-three-zoning-priorities/report.json';review_path=ROOT/'research/next-three-zoning-priorities/report.json'
    ...
    if ids!={27224591,27320676,27243652} or len(cases)!=3:raise ValueError('Wrong acceptance cohort')
    ...
    report={'evidence_state':'DERIVED','method':'scripts/prepare_next_three_zoning_acceptance.py','method_sha256':digest(Path(__file__).read_bytes()),'source_audit_sha256':COUNTY,'proposal_audit_sha256':PROPOSAL,'review_audit_sha256':REVIEW,'inputs':inputs,'baseline_snapshot_id':baseline['snapshot']['snapshot_id'],'corrections':rows,'expected_original_zoning_holds':162,'expected_effective_zoning_holds':151,'expected_original_total_holds':183,'expected_effective_total_holds':172,'qualified_for_parcel_screening':False,'limits':['Geometry interpretations only, not legal zoning or surveyed boundaries.','Other county holds, municipal applicability, parcel identity and wetland age remain unresolved.','Three additional exact versions only; preserve the prior eight corrections. New captures require new review.']}
    ...
    sql.append("DO $$ DECLARE n integer; BEGIN SELECT count(*) INTO n FROM ingest.county_geometry_correction WHERE source_audit_sha256="+literal(COUNTY)+" AND object_id IN (27224591,27320676,27243652); IF n=0 THEN IF NOT ingest.county_geometry_snapshot_is_current("+literal(baseline['snapshot']['snapshot_id'])+","+literal(COUNTY)+") THEN RAISE EXCEPTION 'Acceptance baseline changed'; END IF; ELSIF n<>3 OR NOT EXISTS(SELECT 1 FROM ingest.audit_result WHERE sha256="+literal(audit)+") THEN RAISE EXCEPTION 'Partial acceptance requires review'; END IF; END $$;")
