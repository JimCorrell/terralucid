...
from investigate_next_three_zoning_priorities import BASE,AUDIT,RANKING_SHA,IDS
...
def stage(output):
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_next_three_zoning_priorities.py',ROOT/'tests/test_moosehead_spencer.py',ROOT/'tests/test_moosehead_spencer_review.py']:archive(p.read_bytes())
