"""Upload without overwrite and download/hash every private soil investigation object.
...
from verify_county_soil_bundle import file_sha
PROJECT='yytsjlmbyqhcqfalbjca';BUCKET='terralucid-source-snapshots'
def transfer(evidence,analysis,out,cli):
    report=json.loads((analysis/'report.json').read_bytes())
    objects={}
    for name,entry in [('live-representations',report['live_evidence']),('native-containment',report['native_containment']['request'])]:
        path=evidence/(name+'.response')
        if file_sha(path)!=entry['sha256']:raise ValueError('Source response changed')
        objects[file_sha(path)]=path
        path=evidence/(name+'-request.json');objects[file_sha(path)]=path
    for row in report['candidates']:
        path=analysis/(row['mupolygonkey']+'.candidate.wkb')
        if file_sha(path)!=row['candidate_sha256']:raise ValueError('Candidate changed')
        objects[file_sha(path)]=path
    for name in ['report.json','containment-controls.json','original-gap.wkb','residual-gap.wkb']:
        path=analysis/name;objects[file_sha(path)]=path
    ...
    report={'status':'PASS','project_ref':PROJECT,'bucket':BUCKET,'bucket_private':True,'credential_lookups':1,
            'verified_at':datetime.now(timezone.utc).isoformat(),'investigation_report_sha256':file_sha(analysis/'report.json'),
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--evidence',type=Path,required=True);p.add_argument('--analysis',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--cli',type=Path,required=True);a=p.parse_args();transfer(a.evidence,a.analysis,a.output,a.cli)
