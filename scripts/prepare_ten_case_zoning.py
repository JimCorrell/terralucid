#!/usr/bin/env python3
"""Stage source observations, eleven proposals and a finding review; never activate."""
...
from investigate_ten_case_zoning import BASE,AUDIT,RANKING_SHA,IDS
...
def stage(output):
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_ten_case_zoning.py',ROOT/'tests/test_moosehead_spencer.py',ROOT/'tests/test_moosehead_spencer_review.py']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','scope':'Eleven source-preserving proposals and separate cycle/fill checks; no activation','geometry_snapshot_id':r['geometry_snapshot_id']})])+') ON CONFLICT DO NOTHING;')
