#!/usr/bin/env python3
"""Archive county readiness evidence and append the existing county finding."""
...
from assess_county_readiness import BASE
...
def stage(output):
    ...
    if r['source_audit_sha256']!=AUDIT or r['ranking_audit_sha256']!=RANKING_SHA:raise ValueError('Changed scope')
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),ROOT/'scripts/prepare_county_ranking_refresh.py',BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_county_readiness.py']:archive(p.read_bytes())
    ...
    for log in sorted((BASE/'runs').glob('*/results.json')):
        for e in json.loads(log.read_bytes()):
            ...
            if sha!=e['sha256'] or len(b)!=e['bytes']:raise ValueError('Changed response')
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/orneville','scope':'County readiness and Orneville scanned FEMA products reviewed; parcel applicability unknown; no geometry changes','geometry_snapshot_id':r['geometry_snapshot_id']})])+') ON CONFLICT DO NOTHING;')
    event={k:v for k,v in plan.items() if k!='expected_event_id'};event['event_id']=digest(canonical([audit,event]).encode());event['evidence_refs']=['audit:'+audit+'#/orneville','audit:'+audit+'#/review_conclusion']
