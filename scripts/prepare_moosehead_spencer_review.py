#!/usr/bin/env python3
"""Archive the fixed-candidate review; append findings without accepting geometry."""
import argparse,json
from pathlib import Path
from prepare_source_staging import ROOT,digest,canonical,literal,jsql,BUCKET
from review_moosehead_spencer import BASE,PARENT,PARENT_SHA,REGISTRY
...
def stage(output):
    ...
    for path,sha in r['inputs'].items():
        ...
        archive(b)
    for p in [ROOT/r['method'],Path(__file__),BASE/'finding-review.json',BASE/'test-results.json',ROOT/'tests/test_moosehead_spencer_review.py',REGISTRY/'registry.json']:archive(p.read_bytes())
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest(canonical([audit,'TL-F-0029']).encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/cases','proposal_audit_sha256':PARENT_SHA,'scope':'Both fixed proposals supported for exact-version acceptance; no activation'})])+') ON CONFLICT DO NOTHING;')
