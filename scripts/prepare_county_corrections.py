#!/usr/bin/env python3
"""Prepare three reviewed county acceptances, preserving all original records."""
import argparse,csv,json
from pathlib import Path
from prepare_source_staging import ROOT,digest,canonical,literal,jsql,BUCKET

BASE=ROOT/'research/county-corrections'
COUNTY='fad71a48eab71fce398a8e51bd3fb4e9b7169e1d0e0f2a6ed91b1a18a5fad259'
PROPOSAL='25f03b37818d1fc5cf25a6fa1e9264d48824c8585dbbf9776574dd6682905b6c'
REVIEW='b7d02438499be17b55445bb5b2c5cd621cbe6f24182d238569524e0c9ab20cb7'
MIGRATION='supabase/migrations/20260921050000_county_geometry_corrections.sql'

def prepare(output,transfer):
    proposal_path=ROOT/'research/piscataquis-zoning-holds/report.json';review_path=ROOT/'research/piscataquis-zoning-review/report.json'
    ...
    report={'evidence_state':'DERIVED','method':'scripts/prepare_county_corrections.py','method_sha256':digest(Path(__file__).read_bytes()),'source_audit_sha256':COUNTY,'proposal_audit_sha256':PROPOSAL,'review_audit_sha256':REVIEW,'inputs':inputs,'corrections':rows,'expected_original_zoning_holds':162,'expected_effective_zoning_holds':159,'expected_original_total_holds':183,'expected_effective_total_holds':180,'qualified_for_parcel_screening':False,'limits':['Geometry interpretations only, not legal zoning or surveyed boundaries.','Other county holds, municipal applicability, parcel identity and wetland age remain unresolved.','Three exact versions only; new captures require new review.']}
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest((audit+':TL-F-0029').encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/corrections','note':'Three exact-version acceptances; original holds retained, other county qualification remains open.'})])+') ON CONFLICT DO NOTHING;')
