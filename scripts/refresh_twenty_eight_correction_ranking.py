#!/usr/bin/env python3
"""Versioned ranking refresh after twenty-eight accepted county corrections."""
import json
...
from refresh_county_ranking import AUDIT,OLD_SHA,ACTIVATIONS as FIRST_ACTIVATIONS,parts,newly_covered,refreshed_area,jurisdiction_reductions,validate_dependencies
BASE=ROOT/'research/twenty-eight-correction-ranking'
...
ACTIVATIONS=ACTIVATIONS|{'research/ten-case-acceptance/report.json':'c4a72f6e0674e204416c9f3205314dbc42166f73d66cb1456441655158d47b30'}
PREVIOUS_SHA='8b5f2fb7041826aea92ac697489fc8dea3308df3ffffd3384e31ff6e27f5b6e7'
...
def run():
    ...
    previous=json.loads(read('research/seventeen-correction-ranking/report.json',PREVIOUS_SHA))
    ...
    d=json.loads(read('research/twenty-eight-correction-ranking/dependencies.json'))
    ...
    for path,sha in ACTIVATIONS.items():
        ...
        for c in a['corrections']:
            ...
            parent={'county-corrections':'piscataquis-zoning-holds','moosehead-spencer-acceptance':'moosehead-spencer','three-zoning-acceptance':'three-zoning-priorities','next-three-zoning-acceptance':'next-three-zoning-priorities','latest-three-zoning-acceptance':'latest-three-zoning-priorities','big-moose-t2-barnard-acceptance':'big-moose-t2-barnard','ten-case-acceptance':'ten-case-zoning-review'}[Path(path).parent.name]
    ...
    # Preserve the reviewed baseline partition; add only the new cohort's
    # residual contribution. A global union changes noding order at nearly
    # coincident source segments, producing small area differences.
    prior_count=len(previous['removed_accepted_object_ids'])
    prior_candidates=candidates[:prior_count]
    added=newly_covered(gap,prior_candidates)
    if abs(sum(g.area for g in added)-previous['newly_covered_m2'])>0.01:raise ValueError('Changed baseline partition')
    prior_parts=[p for g in prior_candidates for p in parts(g)];prior_tree=STRtree(prior_parts)
    for p in newly_covered(gap,candidates[prior_count:]):
        hits=prior_tree.query(p,predicate='intersects')
        residual=p.difference(unary_union([prior_parts[int(i)] for i in hits])) if len(hits) else p
        added.extend(parts(residual))
    global_union_area=sum(g.area for g in newly_covered(gap,candidates))
    numerical_difference=global_union_area-sum(g.area for g in added)
    tree=STRtree(added);rows=[]
    for rank,r in enumerate(old['ranking'],1):
        ...
        if additional < -0.01:raise ValueError('Coverage regressed from seventeen-correction ranking')
        rows.append({**r,'seventeen_correction_rank':prior['rank'],'seventeen_correction_envelope_uncovered_county_m2':prior['envelope_uncovered_county_m2'],'additional_reduction_since_seventeen_corrections_m2':max(0.,additional),'previous_rank':rank,'previous_envelope_uncovered_county_m2':r['envelope_uncovered_county_m2'],'accepted_coverage_reduction_m2':reduction,'envelope_uncovered_county_m2':refreshed_area(r['envelope_uncovered_county_m2'],reduction),'potential_gap_by_civil_name_m2':jurisdictions})
    rows.sort(key=lambda r:(-r['envelope_uncovered_county_m2'],r['object_id']))
    for i,r in enumerate(rows,1):r['rank']=i
    report={'evidence_state':'DERIVED','method':'scripts/refresh_twenty_eight_correction_ranking.py','method_sha256':digest(Path(__file__).read_bytes()),'inputs':inputs,'source_audit_sha256':AUDIT,'historical_ranking_sha256':OLD_SHA,'seventeen_correction_ranking_sha256':PREVIOUS_SHA,'additional_coverage_since_seventeen_corrections_m2':sum(g.area for g in added)-previous['newly_covered_m2'],'geometry_snapshot_id':d['snapshot']['snapshot_id'],'geometry_dependencies':d['snapshot']['dependencies'],'calculation_order_check':{'method':'Preserve seventeen-correction coverage partition; append eleven-proposal residuals after subtracting accepted baseline.','global_union_area_m2':global_union_area,'global_union_minus_incremental_m2':numerical_difference,'note':'Nearly coincident source linework is sensitive to GEOS noding order; no source geometry is snapped or changed.'},'runtime':{'shapely':shapely.__version__,'GEOS':shapely.geos_version_string},'original_held_zoning_rows':162,'effective_held_zoning_rows':len(rows),'removed_accepted_object_ids':sorted(c['object_id'] for c in corrections),'original_gap_m2':gap.area,'newly_covered_m2':sum(g.area for g in added),'effective_gap_m2':refreshed_area(gap.area,sum(g.area for g in added)),'qualified_for_parcel_screening':False,'ranking':rows,'limits':['The five PR46 map-date follow-ups (Beaver Cove, Orneville, Bowdoin College Grant West, T1 R13 and Shawtown) remain open, along with the prior Big Moose and T1 R11 discrepancies.','Big Moose map-index amendment date 2016-01-30 versus PDF through 2022-12-30 and T1 R11 map-index amendment date 2005-08-18 versus PDF ZP770 effective 2018-04-26 remains unresolved; geometry acceptance does not establish legal currency.','Envelope/gap overlap is an upper bound, not polygon coverage; priorities overlap and must not be added.','Inventory coverage includes retained source flags; a geometry gap does not establish absent zoning or municipal applicability.','This ranks geometry investigations, not parcels, suitability, legal zoning or acquisition readiness.','Original evidence is preserved; only the twenty-eight accepted exact versions affect this refresh. Check the snapshot against the intended audit before use.']}
