#!/usr/bin/env python3
"""Prepare eleven additional reviewed county acceptances, preserving all original records."""
import argparse,csv,json
from pathlib import Path
from prepare_source_staging import ROOT,digest,canonical,literal,jsql,BUCKET

BASE=ROOT/'research/ten-case-acceptance'
COUNTY='fad71a48eab71fce398a8e51bd3fb4e9b7169e1d0e0f2a6ed91b1a18a5fad259'
PROPOSAL='7f8aa420e8ad488e1cb4752b781488ef1489783282ab782bff99d109992f0221'
REVIEW='7f8aa420e8ad488e1cb4752b781488ef1489783282ab782bff99d109992f0221'
MIGRATION='supabase/migrations/20260922040000_ten_case_acceptance.sql'

def prepare(output,transfer):
    proposal_path=ROOT/'research/ten-case-zoning-review/report.json';review_path=ROOT/'research/ten-case-zoning-review/report.json'
    ...
    if ids!={27342226,27365331,27364426,27339379,27364538,27324664,27321394,27366878,27261431,27304932,27358396} or len(cases)!=11:raise ValueError('Wrong acceptance cohort')
    ...
    report={'evidence_state':'DERIVED','method':'scripts/prepare_ten_case_acceptance.py','method_sha256':digest(Path(__file__).read_bytes()),'source_audit_sha256':COUNTY,'proposal_audit_sha256':PROPOSAL,'review_audit_sha256':REVIEW,'inputs':inputs,'baseline_snapshot_id':baseline['snapshot']['snapshot_id'],'corrections':rows,'expected_original_zoning_holds':162,'expected_effective_zoning_holds':134,'expected_original_total_holds':183,'expected_effective_total_holds':155,'qualified_for_parcel_screening':False,'limits':['Geometry interpretations only, not legal zoning or surveyed boundaries.','Other county holds, municipal applicability, parcel identity and wetland age remain unresolved. The five PR46 map-date follow-ups (Beaver Cove, Orneville, Bowdoin College Grant West, T1 R13 and Shawtown) remain open. Big Moose index amendment date 2016-01-30 versus PDF amendments through 2022-12-30 and T1 R11 index amendment date 2005-08-18 versus PDF ZP770 effective 2018-04-26 remains open; geometry acceptance does not resolve legal currency.','Eleven additional exact versions only; preserve the prior seventeen corrections. New captures require new review.']}
    ...
    sql.append("DO $$ DECLARE n integer; BEGIN SELECT count(*) INTO n FROM ingest.county_geometry_correction WHERE source_audit_sha256="+literal(COUNTY)+" AND object_id IN (27342226,27365331,27364426,27339379,27364538,27324664,27321394,27366878,27261431,27304932,27358396); IF n=0 THEN IF NOT ingest.county_geometry_snapshot_is_current("+literal(baseline['snapshot']['snapshot_id'])+","+literal(COUNTY)+") THEN RAISE EXCEPTION 'Acceptance baseline changed'; END IF; ELSIF n<>11 OR NOT EXISTS(SELECT 1 FROM ingest.audit_result WHERE sha256="+literal(audit)+") THEN RAISE EXCEPTION 'Partial acceptance requires review'; END IF; END $$;")
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest((audit+':TL-F-0029').encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/corrections','note':'Eleven additional exact-version acceptances; original holds retained, other county qualification remains open.'})])+') ON CONFLICT DO NOTHING;')
