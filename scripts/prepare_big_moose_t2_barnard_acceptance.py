...
BASE=ROOT/'research/big-moose-t2-barnard-acceptance'
COUNTY='fad71a48eab71fce398a8e51bd3fb4e9b7169e1d0e0f2a6ed91b1a18a5fad259'
PROPOSAL='eca57f7cce399a9f4de70d6096c4adf002f79d5528927983374e19d51c5d6a50'
REVIEW='eca57f7cce399a9f4de70d6096c4adf002f79d5528927983374e19d51c5d6a50'
MIGRATION='supabase/migrations/20260922030000_big_moose_t2_barnard_acceptance.sql'

def prepare(output,transfer):
    proposal_path=ROOT/'research/big-moose-t2-barnard/report.json';review_path=ROOT/'research/big-moose-t2-barnard/report.json'
    ...
    if ids!={27257109,27321193,27303187} or len(cases)!=3:raise ValueError('Wrong acceptance cohort')
    ...
    report={'evidence_state':'DERIVED','method':'scripts/prepare_big_moose_t2_barnard_acceptance.py','method_sha256':digest(Path(__file__).read_bytes()),'source_audit_sha256':COUNTY,'proposal_audit_sha256':PROPOSAL,'review_audit_sha256':REVIEW,'inputs':inputs,'baseline_snapshot_id':baseline['snapshot']['snapshot_id'],'corrections':rows,'expected_original_zoning_holds':162,'expected_effective_zoning_holds':145,'expected_original_total_holds':183,'expected_effective_total_holds':166,'qualified_for_parcel_screening':False,'limits':['Geometry interpretations only, not legal zoning or surveyed boundaries.','Other county holds, municipal applicability, parcel identity and wetland age remain unresolved. Big Moose index amendment date 2016-01-30 versus PDF amendments through 2022-12-30 and T1 R11 index amendment date 2005-08-18 versus PDF ZP770 effective 2018-04-26 remains open; geometry acceptance does not resolve legal currency.','Three additional exact versions only; preserve the prior fourteen corrections. New captures require new review.']}
    ...
    sql.append("DO $$ DECLARE n integer; BEGIN SELECT count(*) INTO n FROM ingest.county_geometry_correction WHERE source_audit_sha256="+literal(COUNTY)+" AND object_id IN (27257109,27321193,27303187); IF n=0 THEN IF NOT ingest.county_geometry_snapshot_is_current("+literal(baseline['snapshot']['snapshot_id'])+","+literal(COUNTY)+") THEN RAISE EXCEPTION 'Acceptance baseline changed'; END IF; ELSIF n<>3 OR NOT EXISTS(SELECT 1 FROM ingest.audit_result WHERE sha256="+literal(audit)+") THEN RAISE EXCEPTION 'Partial acceptance requires review'; END IF; END $$;")
