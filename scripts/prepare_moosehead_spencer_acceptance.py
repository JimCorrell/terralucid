#!/usr/bin/env python3
"""Prepare two additional reviewed county acceptances, preserving all original records."""
import argparse,csv,json
from pathlib import Path
from prepare_source_staging import ROOT,digest,canonical,literal,jsql,BUCKET

BASE=ROOT/'research/moosehead-spencer-acceptance'
COUNTY='fad71a48eab71fce398a8e51bd3fb4e9b7169e1d0e0f2a6ed91b1a18a5fad259'
PROPOSAL='959a971c95dc6e81a1179d26f345eba9dc2fe9b908c40107f43272b27ef5d49a'
REVIEW='6a649c0cfffedf0fde0d3790a6f44fc8a0990baa9e5e5b7e705f7569e513900a'
MIGRATION='supabase/migrations/20260921060000_moosehead_spencer_acceptance.sql'

def prepare(output,transfer):
    proposal_path=ROOT/'research/moosehead-spencer/report.json';review_path=ROOT/'research/moosehead-spencer-review/report.json'
    ...
    p=json.loads(proposal_path.read_bytes());review=json.loads(review_path.read_bytes());cases=[c for c in p['cases'] if c['status']=='proposed_not_accepted'];ids={c['object_id'] for c in cases}
    ...
    inputs={}
    for path in [proposal_path,review_path,ROOT/MIGRATION,Path(__file__),ROOT/'research/piscataquis-ingestion/transport-verification.json']:
    ...
    report={'evidence_state':'DERIVED','method':'scripts/prepare_moosehead_spencer_acceptance.py','method_sha256':digest(Path(__file__).read_bytes()),'source_audit_sha256':COUNTY,'proposal_audit_sha256':PROPOSAL,'review_audit_sha256':REVIEW,'inputs':inputs,'corrections':rows,'expected_original_zoning_holds':162,'expected_effective_zoning_holds':157,'expected_original_total_holds':183,'expected_effective_total_holds':178,'qualified_for_parcel_screening':False,'limits':['Geometry interpretations only, not legal zoning or surveyed boundaries.','Other county holds, municipal applicability, parcel identity and wetland age remain unresolved.','Two additional exact versions only; preserve the first three corrections. New captures require new review.']}
    ...
    sql.append('INSERT INTO ingest.finding_occurrence(occurrence_id,finding_id,audit_sha256,details) VALUES ('+','.join([literal(digest((audit+':TL-F-0029').encode())),literal('TL-F-0029'),literal(audit),jsql({'report_pointer':'/corrections','note':'Two additional exact-version acceptances; original holds retained, other county qualification remains open.'})])+') ON CONFLICT DO NOTHING;')
