#!/usr/bin/env python3
"""Eleven exact-source proposals with separate cycle/fill checks; no activation."""
import json
...
from refresh_county_ranking import parts,newly_covered,validate_dependencies,AUDIT
from refresh_seventeen_correction_ranking import ACTIVATIONS
from investigate_katahdin_contacts import propose_katahdin,review_katahdin_cycles

BASE=ROOT/'research/ten-case-zoning-review'
RANKING='research/seventeen-correction-ranking/report.json'
RANKING_SHA='8b5f2fb7041826aea92ac697489fc8dea3308df3ffffd3384e31ff6e27f5b6e7'
IDS=[27342226,27365331,27364426,27339379,27364538,27324664,27321394,27366878,27261431,27304932,27358396]
ASSESSMENT_SHA="eb861bfcd63c1eeadb02d2761f2516370446544b3b4b568f0c44c5f48e93f82e"
...
def build():
    ...
    assessment=json.loads(checked('research/county-backlog-assessment/report.json',ASSESSMENT_SHA))
    if ranking['source_audit_sha256']!=AUDIT or assessment['scenario_priority_ids'][:10]+[27358396]!=IDS:raise ValueError('Wrong assessed cohort')
    ...
    for p,h in ACTIVATIONS.items():
        ...
        for c in activation['corrections']:
            corrections.append(c);parent={'county-corrections':'piscataquis-zoning-holds','moosehead-spencer-acceptance':'moosehead-spencer','three-zoning-acceptance':'three-zoning-priorities','next-three-zoning-acceptance':'next-three-zoning-priorities','latest-three-zoning-acceptance':'latest-three-zoning-priorities','big-moose-t2-barnard-acceptance':'big-moose-t2-barnard'}[Path(p).parent.name]
    ...
    for oid in IDS:
        ...
        rings=original['geometry']['rings'];candidate,details=(propose_katahdin(rings) if oid==27358396 else propose(rings))
    ...
    report={'method':'scripts/investigate_ten_case_zoning.py','method_sha256':digest(Path(__file__).read_bytes()),'evidence_state':'DERIVED','source_audit_sha256':AUDIT,'ranking_audit_sha256':RANKING_SHA,'assessment_sha256':ASSESSMENT_SHA,'map_date_followups':maps['new_date_discrepancies'],'retained_prior_followups':maps['retained_prior_followups'],'geometry_snapshot_id':deps['snapshot']['snapshot_id'],'geometry_dependencies':deps['snapshot']['dependencies'],'inputs':inputs,'evidence':evidence,'runtime':{'shapely':shapely.__version__,'GEOS':shapely.geos_version_string,'numpy':np.__version__},'cases':cases,'proposed_combined_additional_gap_m2':scenario_area(gap,candidates,accepted),'scenario_method':'Union of proposal intersections with the original valid-zoning gap, minus union of all seventeen accepted candidates; spatially indexed exact overlay.','review_conclusion':'All eleven exact-source proposals preserve complete segment multiplicity, source cycle roles and independent even-odd fill. Supported for reviewable exact-version activation after a tested extension of the bounded county validator.','limits':['No activation: all 145 zoning and 21 parcel holds remain; seventeen prior acceptances are unchanged.','Distinct cycle/fill algorithms use the same GEOS engine and are not publisher approval or survey evidence.','Five new map date discrepancies and the prior Big Moose and T1 R11 discrepancies remain open; geometry acceptance cannot establish legal currency.','Official maps provide context and limitations, not exact contact coordinates or legal currency.','Scenario is potential usable inventory geometry, not absent zoning, buildability or qualified parcel screening.']}
