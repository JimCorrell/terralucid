"""Upload without overwrite and download/hash every private soil metadata evidence object.
...
def transfer(evidence,report,out,cli):
    manifest=json.loads((evidence/'sources.json').read_bytes());objects={}
    for entry in manifest:
        path=evidence/entry['file']
        if file_sha(path)!=entry['sha256']:raise ValueError('Metadata evidence changed')
        objects[entry['sha256']]=path
    for path in [evidence/'sources.json',report]:objects[file_sha(path)]=path
    ...
    report={'status':'PASS','project_ref':PROJECT,'bucket':BUCKET,'bucket_private':True,'credential_lookups':1,
            'verified_at':datetime.now(timezone.utc).isoformat(),'metadata_report_sha256':file_sha(report),
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--evidence',type=Path,required=True);p.add_argument('--report',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--cli',type=Path,required=True);a=p.parse_args();transfer(a.evidence,a.report,a.output,a.cli)
